...
#define AE              cout << fixed << setprecision(15);
#define ld              double
#define f               first
#define ss              second
...
void build(ll u, ll st, ll en) {
    ...
    if (st==en) {
        ...
        ll cnt = min ( bame.open , dane.close );

        res.oc = bame.oc + dane.oc + cnt + cnt;
        res.open = bame.open + dane.open - cnt;
        res.close = bame.close + dane.close - cnt;
        tr[u] = res;
    }
}

void update(ll u, ll st, ll en, ll l, ll r) {
    Node bame , dane , res;
    if (r<st || en<l)  return;
    else if (l<=st && en<=r) {
        if ( s[st] == '(' ) s[st] = ')';
...
void Solution ( int tc ){
  ...
  for ( int i = 1 ; i <= q ; i++ ) {
    int x;
    cin >> x;
    if ( x == 0 ) cout << (query ( 1 , 1 , n , 1 , n ).oc == n ? "YES" : "NO")  << "\n";
    else update ( 1 , 1 , n , x , x );
  }
