...
int main()
{
  ...
  for ( ll i = 0 ; i<humburger.size() ; i++ ){
      needB+=humburger[i]=='B';
      needS+=humburger[i]=='S';
      needC+=humburger[i]=='C';
  }

 ///  dbg ( needB , needS , needC );
  ...
  while ( l<=r ){
       ...
       ll costforB = max ( 0LL , ((mid*needB) - numB)*priceB);
       ll costforS = max ( 0LL , ((mid*needS) - numS)*priceS);
       ll costforC = max ( 0LL , ((mid*needC) - numC)*priceC);

       ll totalcost = costforB + costforS + costforC;
