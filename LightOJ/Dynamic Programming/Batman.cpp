...
int recur ( int i , int j , int k ){
    ...
    if ( vis[i][j][k] == cur ) return dp[i][j][k];
    vis[i][j][k] = cur;

    int mx = 0;

    if ( a[i] == b[j] and b[j] == c[k] ) mx = max ( mx , 1 + recur (i+1,j+1,k+1));
    else{
        if(i+1<=n) mx = max ( mx , recur (i+1,j,k));
        if(j+1<=m) mx = max ( mx , recur (i,j+1,k));
        if(k+1<=o) mx = max ( mx , recur (i,j,k+1));
    }
