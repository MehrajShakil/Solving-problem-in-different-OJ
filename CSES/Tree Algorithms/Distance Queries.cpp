...
int Node;
int dep[N];
int kthAncestor[N][21];
int q;
int deg[N];

void dfs( int node , int d , int par = -1 ){
 dep[node] = d;
 kthAncestor[node][0] = par; /// set first parent.
  ...
  for ( int i = 1 ; 1<<i <=Node ; ++i ) {
    for ( int j = 1 ; j<=Node ; ++j ) {
        if ( kthAncestor[j][i-1]!=-1 ) {
            int par = kthAncestor[j][i-1];
            kthAncestor[j][i] = kthAncestor[par][i-1];
        }
    }
  }
}
...
int LCA(int x, int y)
{
    ...
    for(int j=20; j>=0; j--) {
        if(kthAncestor[x][j]!=kthAncestor[y][j]) {
            x=kthAncestor[x][j], y=kthAncestor[y][j];
        }
    }
    return kthAncestor[x][0];
}
