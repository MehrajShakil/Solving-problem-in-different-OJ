...
Node combine(Node a, Node b) {
    Node res;
    res.mx = max(0LL,max(a.mx,max(b.mx,a.total+b.prefMx)));
    res.prefMx = max(0LL,max(a.prefMx,a.total+b.prefMx));
...
void build(int node, int l, int r) {
    if(l==r) {
        tr[node] = { max(0LL,arr[l]), max(0LL,arr[l]), arr[l] };
    }
    else {
        int mid = (l+r)>>1;
        int lc = node*2;
