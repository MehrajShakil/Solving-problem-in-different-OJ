...
struct Node{
    ll mx, pre, suff, total;
} tr[N<<2];

Node go(Node a, Node b) {
     Node res;
     res.mx = max(0LL,max(max(a.mx,b.mx),a.suff+b.pre));
     res.pre = max(0LL,max(a.total+b.pre,a.pre));
     res.suff = max(0LL,max(b.total+a.suff,b.suff));
...
void build(int node, int l, int r) {
     if(l == r) {
        tr[node].mx = max(0LL,arr[l]);
        tr[node].pre = max(0LL,arr[l]);
        tr[node].suff = max(0LL,arr[l]);
        tr[node].total = arr[l];
     }
     else {
        ll mid = (l+r)>>1;
