...
Node combine(Node a, Node b) {
    Node temp;
    temp.mnLeft = min(a.mnLeft,b.mnLeft);
    temp.mnRight = min(a.mnRight,b.mnRight);
    return temp;
}
void build(int node, int l, int r) {
    if(l == r) {
      tr[node].mnLeft = arr[l]+l;
      tr[node].mnRight = arr[l] + (n-l+1);
    }
    else {
        int mid = (l+r)>>1;
        int lc = node*2;
...
void Solution ( int tc ){
   ...
   for ( int i = 1 ; i <= q ; ++i ) {
    ...
    if(task==1) {
        ...
        cin >> k;
        int x = query(1, 1, n, k, n).mnLeft;
        int mn = x - k;
        x = query(1, 1, n, 1, k).mnRight;
        mn = min(mn, (x-(n-k+1)));
        cout << mn << "\n";
    }
   }
