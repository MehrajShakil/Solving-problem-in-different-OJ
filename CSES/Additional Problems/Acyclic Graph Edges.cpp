...
int node;
vector<int> vis(N,0);
vector < pair < int , int > > e[N], edge[N+1];

void dfs( int node ){
   vis[node] = 1;
   for ( auto child : e[node] ) {
     if ( vis[child.first] == 1 ) {
        edge[child.second].push_back ( { child.first , node } );
     }
     else if ( vis[child.first] == 0 ) {
        edge[child.second].push_back({node,child.first});
        dfs(child.first);
     }
     else{
        edge[child.second].push_back({node,child.first});
...
void Solution ( int tc ){
 ...
 for ( int i = 1 ; i<=m ; ++i ) {
    cout << edge[i][0].first << " "<< edge[i][0].second << "\n";
 }
